...
public class CertificateValidator {
    ...
    public static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert, Set<X509Certificate> additionalCerts)
        ...
        try {
            ...
            if (isSelfSigned(cert)) {
                throw new CertificateValidationException("The certificate is self-signed.");
            }
            ...
            // Check whether the certificate is revoked by the CRL, which is issued from the OpenXPKI
            new OpenXPKICRLValidator().verifyCertificateCRLs(cert);

            // The chain is built and verified. Return it as a result
            return verifiedCertChain;
        } catch (CertPathBuilderException certPathEx) {
            throw new CertificateValidationException("Error building certification path: " + cert.getSubjectX500Principal(), certPathEx);
        } catch (CertificateValidationException cvex) {
            throw cvex;
        } catch (Exception ex) {
            throw new CertificateValidationException("Error verifying the certificate: " + cert.getSubjectX500Principal(), ex);
        }
    }
    ...
    private static PKIXCertPathBuilderResult verifyCertificate(X509Certificate cert, Set<X509Certificate> trustedRootCerts,
        ...
        // Specify a list of intermediate certificates "BC"
        CertStore intermediateCertStore = CertStore.getInstance("Collection", new CollectionCertStoreParameters(intermediateCerts));
        pkixParams.addCertStore(intermediateCertStore);

        // Build and verify the certification chain "BC"
